...
// 2am UTC Thu → Day 4: Train Heroes        (2 pts)  — hero EXP ONLY, save radar
// 2am UTC Fri → Day 5: Total Mobilization  (2 pts)  — troops, radar tasks
  ...
  switch (day) {
    ...
    case 5:
      actions.push({
        ...
        buddyPrompt: `Today is Alliance Duel Day 5 — Total Mobilization. I'm at HQ ${hq} with ${profile.troop_type} troops at ${profile.troop_tier}. How do I maximize troop training today?`,
        points: 2,
      })
      actions.push({
        id: 'day5_radar',
...
//   - T10 path / T11 armament / defense review / hot deals:
//     ONLY on Days 1, 2, 3, 4, 5 — NOT on Day 6 (battle focus) or Day 7 (reset)
...
export function generateActionPlan(profile: CommanderProfile): ActionPlanResult {
  ...
  // Tier-conditional actions only fire on Days 1–5 (not battle day, not reset day)
  ...
  // ── Hero — Day 4 ONLY ──────────────────────────────────────────────────────
  // HARD RULE: Hero EXP, skill medals, exclusive weapons score on Day 4 only.
  ...
  // None of these fire on Day 6 (battle focus) or Day 7 (reset day)

  // T10 unlock path — non-double days only (Days 1 and reset excluded from double)
  ...
  // T11 Armament — non-double days only
  if (isT11 && allowTierActions && !isDoubleDay) {
    actions.push({
      id: 't11_armament',
      category: 'troops',
      priority: 'high',
  ...
  // Defense review — endgame only, Days 1–5, non-double days
  if (isEndGame && allowTierActions && !isDoubleDay) {
    actions.push({
      id: 'defense_review',
      category: 'defense',
      priority: 'low',
...
function getAdvancedStrategicInsight(
  ...
  return nextMilestone
    ...
    : `Heroes at max unlock level ${maxHeroLevel} for your HQ. Focus on Armament Research and troop tier progression.`
}
